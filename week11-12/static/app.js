...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.deleteArticle = async (id) => {
    if (
      !confirm(
        `Bạn có chắc chắn muốn xóa bài viết #${id}? (Thao tác này sẽ phát đi sự kiện article.deleted)`,
      )
    )
      return;
  ...
  articleForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      fetchArticles();
      showLocalToast(
        id ? "Cập nhật bài viết thành công!" : "Tạo bài viết mới thành công!",
      );
    } catch (error) {
      alert(`Lỗi: ${error.message}`);
    }
  ...
  function renderSubscriptions(items) {
    ...
    subscriptionsList.innerHTML = items
      .map((sub) => {
        const eventsHtml = sub.events
          .map((ev) => `<span class="sub-event-tag">${ev}</span>`)
          .join("");
        return `
                <tr>
                    <td>
  ...
  window.deleteSubscription = async (subId) => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/webhooks/subscriptions/${subId}`,
        {
          method: "DELETE",
        },
      );
  ...
  subscribeForm.addEventListener("submit", async (e) => {
    ...
    const checkedEvents = [];
    document
      .querySelectorAll('input[name="sub-events"]:checked')
      .forEach((cb) => {
  ...
  async function fetchNotifications(silent = false) {
    try {
      ...
      // Avoid recreating DOM elements if notifications list hasn't changed
  ...
  function renderNotifications(items) {
    ...
    // Process items in order of creation (oldest to newest).
    // Prepending them one by one will naturally place the newest items at the top.
    items.forEach((noti) => {
      const existingEl = document.getElementById(`noti-${noti.id}`);
      if (existingEl) {
        return; // Skip already rendered notifications to prevent animation/flicker
      }
      ...
      const formattedTime = new Date(noti.timestamp).toLocaleTimeString(
        "vi-VN",
      );
      const isExpanded = expandedNotificationIds.has(noti.id);
      ...
      notiEl.id = `noti-${noti.id}`;
      notiEl.className = `notification-item ${statusClass}`;
      notiEl.innerHTML = `
                ...
                <div>
                    <button class="noti-details-btn" onclick="togglePayload(this, '${noti.id}')">${btnText}</button>
                    <pre class="noti-payload" style="display: ${displayStyle}">${escapeHtml(JSON.stringify(noti, null, 2))}</pre>
                </div>
            `;

      // Prepend to show newest at the top
      notificationsList.insertBefore(notiEl, notificationsList.firstChild);
    });
  }

  window.togglePayload = (btn, notiId) => {
    const pre = btn.nextElementSibling;
    if (pre.style.display === "block") {
      pre.style.display = "none";
      btn.textContent = "Xem chi tiết Payload";
      expandedNotificationIds.delete(notiId);
    } else {
      pre.style.display = "block";
      btn.textContent = "Ẩn chi tiết Payload";
      expandedNotificationIds.add(notiId);
    }
  };
